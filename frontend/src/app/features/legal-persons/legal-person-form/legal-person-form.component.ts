  ...
  template: `
    ...
    <div class="card">
      <div class="header-section">
        <h2>{{ isEditMode ? 'Editar' : 'Nova' }} Pessoa Jurídica</h2>
      </div>
        ...
        <div class="form-grid">
          <div class="form-field">
            ...
            <input 
              ...
              [class.p-invalid]="form.get('companyName')?.invalid && form.get('companyName')?.touched"
              class="w-full" />
            <small class="p-error" *ngIf="form.get('companyName')?.invalid && form.get('companyName')?.touched">
              <i class="pi pi-exclamation-circle"></i> Razão Social é obrigatória
            </small>
          </div>
          ...
          <div class="form-field">
            ...
            <input 
              ...
              [class.p-invalid]="form.get('tradeName')?.invalid && form.get('tradeName')?.touched"
              class="w-full" />
            <small class="p-error" *ngIf="form.get('tradeName')?.invalid && form.get('tradeName')?.touched">
              <i class="pi pi-exclamation-circle"></i> Nome Fantasia é obrigatório
            </small>
          </div>
          ...
          <div class="form-field">
            ...
            <small class="p-error" *ngIf="form.get('taxId')?.invalid && form.get('taxId')?.touched">
              <i class="pi pi-exclamation-circle"></i> CNPJ é obrigatório
            </small>
          </div>

          <div class="form-field">
            <label for="foundingDate">Data de Fundação *</label>
            <p-calendar 
              id="foundingDate" 
              formControlName="foundingDate"
              dateFormat="dd/mm/yy"
              [showIcon]="true"
              [styleClass]="form.get('foundingDate')?.invalid && form.get('foundingDate')?.touched ? 'w-full p-invalid' : 'w-full'">
            </p-calendar>
            <small class="p-error" *ngIf="form.get('foundingDate')?.invalid && form.get('foundingDate')?.touched">
              <i class="pi pi-exclamation-circle"></i> Data de fundação é obrigatória
            </small>
          </div>
...
export class LegalPersonFormComponent implements OnInit {
    ...
    this.personService.getById(id).subscribe({
      next: (person) => {
        this.form.patchValue({
          ...person,
          foundingDate: new Date(person.foundingDate)
        });
        this.loading = false;
      },
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: this.getErrorMessage(error, 'Erro ao carregar pessoa jurídica')
        });
        this.loading = false;
      }
    });
  ...
  onSubmit(): void {
    ...
    this.loading = true;
    const person: LegalPerson = this.form.value;
    ...
    operation.subscribe({
      next: () => {
        this.messageService.add({
          severity: 'success',
          summary: 'Sucesso',
          detail: `Pessoa jurídica ${this.isEditMode ? 'atualizada' : 'criada'} com sucesso`
        });
        setTimeout(() => {
          this.router.navigate(['/legal-persons']);
        }, 1000);
      },
      error: (err: any) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: this.getErrorMessage(err, `Erro ao ${this.isEditMode ? 'atualizar' : 'criar'} pessoa jurídica`)
        });
        this.loading = false;
      }
    });
