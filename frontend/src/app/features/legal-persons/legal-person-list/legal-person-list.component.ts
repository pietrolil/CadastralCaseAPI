...
@Component({
  selector: 'app-legal-person-list',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
    ...
    <div class="card">
      ...
      <p-table 
          ...
          <tr>
            ...
            <td>
              <p-button 
                ...
                [text]="true"
                severity="info"
                [routerLink]="['/legal-persons/edit', person.id]">
              </p-button>
              <p-button 
                icon="pi pi-trash" 
                [rounded]="true"
        ...
        <ng-template pTemplate="emptymessage">
          <tr>
            <td colspan="6" class="text-center">Nenhuma pessoa jurídica encontrada.</td>
          </tr>
        </ng-template>
      </p-table>
    </div>
...
export class LegalPersonListComponent implements OnInit {
    ...
    this.personService.getAll().subscribe({
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: this.getErrorMessage(error, 'Erro ao carregar pessoas jurídicas')
        });
        this.loading = false;
      }
    });
  ...
  confirmDelete(person: LegalPerson): void {
    this.confirmationService.confirm({
      message: `Deseja realmente excluir ${person.companyName}?`,
    ...
    this.personService.delete(id).subscribe({
      ...
      error: (error) => {
        this.messageService.add({
          severity: 'error',
          summary: 'Erro',
          detail: this.getErrorMessage(error, 'Erro ao excluir pessoa jurídica')
        });
      }
    });
  }
